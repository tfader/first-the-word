...
DZIECINSTWO = 15             # tyle cykli potomek jest karmiony przez rodziców; potem pępowina odcięta
...
class Byt:
    ...
    def wchlon(self, nr, s):
        ...
        k = k / d
        self.P[nr] += WCHLANIANIE * np.outer(k, k)
    ...
    def chce_isc(self, glod):
        ...
        if self.zjadl >= KOSZT_TRWANIA * 2 and self.zjadl >= self.dobre_ile * 0.7:
            self.dobre_miejsce, self.dobre_ile = self.miejsce, self.zjadl   # zapamiętuje dobre miejsce
    ...
    def potomek(self, partner=None):
        ...
        nowy = self.K.copy()
        if partner is not None and partner.K.shape == self.K.shape:
            od_partnera = np.random.random(len(nowy)) < 0.5
            nowy[od_partnera] = partner.K[od_partnera]
    ...
    # --- cykl ---
    def cykl(self, slowo=None, surowa=None, inny=None, partner=None, dwoje=False, inni=None):
        ...
        for syg in sygnaly:
            if budzet < KOSZT_ODRUCHU:
                break
            o, w, k = self.kaskada(syg[1], budzet, zrodlo="inny")
            ...
            if o:
                ...
                if self.czyn is None:
                    ...
                    if typ == "wolanie" and self.energia > SYTOSC / 2:
                        self.czyn, self.czyn_do = "chodź do mnie", syg[0]         # wrodzone
                    elif typ == "glos" and glod >= GLOD_RUCHU:
                        zr = self.zrodla.get(str(o[0]), {})
                        if zr and max(zr, key=zr.get) == "pokarm":
                            self.czyn, self.czyn_do = "chodź, tu pokarm", syg[0]   # wyuczone
            else:
                typ = syg[2] if len(syg) > 2 else "wyjscie"
                if typ == "glos" and o_p:
        ...
        self.cos = self.zapis("jawa", ilosc, o_r, o_p, o_e, self.wyszlo, self.zjadl, glod=glod,
                              o_s=o_s, slowo=slowo,
                              inny=(self.czyn_do if self.czyn_do is not None else inny[0], o_i,
                                    next((x[2] for x in sygnaly if x[0] == self.czyn_do), inny[2] if len(inny) > 2 else "wyjscie")) if inny else None,
